# TODO: cambiar esto en producción
        ...
        try:
            ...
            if user_profile.check_following(profile):
                formatted_response = {
                    ...
                    "message": f"Tu ya estas siguiendo a {profile.user.first_name} {profile.user.last_name}",
                }
                return Response(formatted_response, status=status.HTTP_400_BAD_REQUEST)
            user_profile.follow(profile)
            subject = "Un nuevo usuario te sigue"
            message = f"HOla, {profile.user.first_name}!!, el usuario {user_profile.user.first_name} {user_profile.user.last_name} ahora te sigue"
            ...
            return Response(
                {
                    "status_code": status.HTTP_200_OK,
                    "message": f"Tu estas ahora siguiendo a {profile.user.first_name} {profile.user.last_name}",
                }
            )
        except Profile.DoesNotExist:
            raise NotFound("Tu no puedes seguir un perfil que no existe.")
...
class UnfollowAPIView(APIView):
    def post(self, request, user_id, *args, **kwargs):
        ...
        if not user_profile.check_following(profile):
            formatted_response = {
                "status_code": status.HTTP_400_BAD_REQUEST,
                "message": f"Tu no puedes dejar de seguir a {profile.user.first_name} {profile.user.last_name} ya que no la estabas siguiendo desde el principio",
            }
            return Response(formatted_response, status=status.HTTP_400_BAD_REQUEST)
        ...
        formatted_response = {
            "status_code": status.HTTP_200_OK,
            "message": f"Tu has dejado de seguir a {profile.user.first_name} {profile.user.last_name}",
        }
        return Response(formatted_response, status=status.HTTP_200_OK)
